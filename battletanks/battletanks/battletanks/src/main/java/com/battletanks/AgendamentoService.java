...
public class AgendamentoService {
    ...
    public static void agendarPartida(Scanner scanner, CadastroService cadastroService) {
        ...
        while (!entradaValida) {
            ...
            try {
                ...
                opcaoModo = Integer.parseInt(input);

                if (opcaoModo >= 1 && opcaoModo <= 4) {
                    entradaValida = true;
                } else {
    ...
    String dataStr = scanner.nextLine();System.out.print("Hora (HH:MM): ");
    String horaStr = scanner.nextLine();

    LocalDateTime dataHora = parseDataHora(dataStr, horaStr);if(dataHora==null)
    {
    ...
    int duracao = scanner.nextInt();scanner.nextLine();
    ...
    // Arena
    System.out.println("Arenas disponíveis:");System.out.println("1. DESERTO");System.out.println("2. URBANO");System.out.println("3. CAMPO_ABERTO");System.out.print("Escolha a arena: ");
    ...
    private static void adicionarTanquesPartida(Scanner scanner, Partida partida, CadastroService cadastroService,
        ...
        while (partida.getTanquesParticipantes().size() < limite) {
            ...
            try {
                ...
                System.out.println("Entrada inválida! Digite um número entre 0 e " + (tanquesDisponiveis.size() - 1)
                        + " ou -1 para terminar.");
            }
        }
    }
            ...
            case 1:
                if ("AGENDADA".equals(partida.getStatus())) {
                    if (partida.getTanquesParticipantes().size() >= getMinimoTanques(partida.getModo())) {
                        ...
                        RankingService.registrarEventoGlobal("Partida #" + partida.getId() + " iniciada - "
                                + partida.getModo() + " na arena " + partida.getArena());
                    } else {
